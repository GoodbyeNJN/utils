...
var Result = class Result {
	...
	static try(fnOrData, onThrow) {
		...
		try {
			...
			return data.then((value) => ok(value), (error) => Err.fromError(transformError(error), Result.try));
		} catch (error) {
...
const safeParse = (text, reviver) => {
	...
	return Result.try(fn, Error).context(`Failed to parse JSON string: ${text.length > 100 ? text.slice(0, 100) + "..." : text}`);
};
...
async function $(cmd, ...values) {
	...
	return (await Result.try(fn, Error)).context(`Failed to execute command: ${cmd}`);
}
