import { isArray, isBoolean, isFunction, isNumber, isPlainObject, isString } from "@/fp";
...
export const joinWith = (separator: string, ...paths: string[]): string => {
    let pathname = "";
...
export const splitBy = (separator: string, path: string): string[] => {
    const paths = [];
...
export const joinWithSlash = (...paths: string[]): string => joinWith("/", ...paths);
export const splitBySlash = (path: string): string[] => splitBy("/", path);

export const splitByLineBreak = (str: string): string[] => str.split(REGEXP_LINE_BREAK);
