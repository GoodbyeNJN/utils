...
import { err, Err, errAsync, ok, Ok, okAsync, Result, ResultAsync } from "./";
...
describe("Result.Ok", () => {
    ...
    describe("andThen", () => {
        it("Maps to an Ok", () => {
            ...
            expect(flattened.value).toStrictEqual({ data: "why not" });
        });
    ...
    describe("inspect", () => {
        it("Calls the passed function but returns an original ok", () => {
            ...
            const teed = okVal.inspect(passedFn);

            expect(teed.isOk()).toBe(true);
            expect(passedFn).toHaveBeenCalledTimes(1);
            expect(teed.value).toStrictEqual(12);
        });
    ...
    describe("inspectErr", () => {
        it("Calls the passed function but returns an original err", () => {
            ...
            const teed = errVal.inspectErr(passedFn);

            expect(teed.isErr()).toBe(true);
            expect(passedFn).toHaveBeenCalledTimes(1);
            expect(teed.error).toStrictEqual(12);
        });
    ...
    describe("orElse", () => {
        it("Skips orElse on an Ok value", () => {
            const okVal = ok(12);
            const errorCallback = vitest.fn(_errVal => err<string>("It is now a string"));
    ...
    it("Maps to a ResultAsync", async () => {
        ...
        const flattened = okVal.andThenAsync(_number => {
            // ...
            // complex async logic
            // ...
        ...
        expect(newResult.value).toStrictEqual({ data: "why not" });
    });
    ...
    it("Maps to a promise", async () => {
        ...
        const promise = okVal.mapAsync(asyncMapper);
        ...
        expect(newResult.value).toStrictEqual("Very Nice!");
    });
...
describe("Result.Err", () => {
    ...
    it("unwrapOr and return the default value", () => {
        const okVal = err<string>("Oh nooo");
    ...
    it("Skips over andThen", () => {
        ...
        const mapper = vitest.fn(_val => ok<string>("yooyo"));

        const hopefullyNotFlattened = errVal.andThen(mapper);

        expect(hopefullyNotFlattened.isErr()).toBe(true);
        expect(mapper).not.toHaveBeenCalled();
        expect(errVal.error).toEqual("Yolo");
    });

    it("Skips over inspect", () => {
        const errVal = err("Yolo");

        const mapper = vitest.fn(_val => {});

        const hopefullyNotFlattened = errVal.inspect(mapper);
    ...
    it("Transforms error into ResultAsync within `andThenAsync`", async () => {
        const errVal = err("Yolo");

        const asyncMapper = vitest.fn(_val => okAsync<string>("yooyo"));

        const hopefullyNotFlattened = errVal.andThenAsync(asyncMapper);
        ...
        expect(syncResult.error).toEqual("Yolo");
    });

    it("Does not invoke callback within `mapAsync`", async () => {
        const asyncMapper = vitest.fn(_val => {
            // ...
            // complex logic
            // ..
        ...
        const promise = errVal.mapAsync(asyncMapper);
        ...
        expect(asyncMapper).not.toHaveBeenCalled();
        expect(sameResult.error).toEqual(errVal.error);
    });
    ...
    it("Unwraps without issue", () => {
        ...
        expect(okVal.error).toBe(12);
        expect(errVal.value).toBe(null);
    });
...
describe("Result.fromThrowable", () => {
    it("Creates a function that returns an OK result when the inner function does not throw", () => {
        ...
        expect(result).toEqual(safeResult.value);
    });
...
describe("Result.all", () => {
    describe("Synchronous `all`", () => {
        it("Combines a list of results into an Ok value", () => {
            const resultList = [ok(123), ok(456), ok(789)];

            const result = Result.all(resultList);

            expect(result.isOk()).toBe(true);
            expect(result.value).toEqual([123, 456, 789]);
        });
        ...
        it("Combines a list of results into an Err value", () => {
            ...
            expect(result.isErr()).toBe(true);
            expect(result.error).toBe("boooom!");
        });
        ...
        it("Combines heterogeneous lists", () => {
            ...
            const result: ExpecteResult = Result.all(heterogenousList);

            expect(result.value).toEqual(["Yooooo", 123, true]);
        });
        ...
        it("Does not destructure / concatenate arrays", () => {
            ...
            const result: ExpectedResult = Result.all(homogenousList);
    ...
    describe("`ResultAsync.all`", () => {
        it("Combines a list of async results into an Ok value", async () => {
            ...
            const resultAsync: ResultAsync<number[], never[]> = ResultAsync.all(asyncResultList);

            expect(resultAsync).toBeInstanceOf(ResultAsync);

            const result = await ResultAsync.all(asyncResultList);
        ...
        it("Combines a list of results into an Err value", async () => {
            ...
            const result = await ResultAsync.all(resultList);
        ...
        it("Combines heterogeneous lists", async () => {
            ...
            const result: ExpecteResult = await ResultAsync.all(heterogenousList);

            expect(result.value).toEqual(["Yooooo", 123, true, [1, 2, 3]]);
        });
    });
});
describe("Result.allSettled", () => {
    describe("Synchronous `allSettled`", () => {
        it("Combines a list of results into an Ok value", () => {
            const resultList = [ok(123), ok(456), ok(789)];

            const result = Result.allSettled(resultList);
        ...
        it("Combines a list of results into an Err value", () => {
            ...
            expect(result.isErr()).toBe(true);
            expect(result.error).toEqual(["boooom!", "ahhhhh!"]);
        });
        ...
        it("Combines heterogeneous lists", () => {
            ...
            const result: ExpecteResult = Result.allSettled(heterogenousList);
        ...
        it("Does not destructure / concatenate arrays", () => {
            ...
            const result: ExpectedResult = Result.allSettled(homogenousList);
    ...
    describe("`ResultAsync.allSettled`", () => {
        it("Combines a list of async results into an Ok value", async () => {
            const asyncResultList = [okAsync(123), okAsync(456), okAsync(789)];

            const result = await ResultAsync.allSettled(asyncResultList);
        ...
        it("Combines heterogeneous lists", async () => {
            ...
            const result: ExpecteResult = await ResultAsync.allSettled(heterogenousList);
...
describe("ResultAsync", () => {
    it("Is awaitable to a Result", async () => {
        ...
        expect(err.error).toEqual("Wrong format");
    });
    ...
    describe("acting as a Promise<Result>", () => {
        ...
        it("Can be used with Promise.all", async () => {
            const allResult = await Promise.all([okAsync<string>("1")]);
            ...
            expect(allResult[0].value).toEqual("1");
        });
    ...
    describe("map", () => {
        ...
        it("Skips an error", async () => {
            const asyncErr = errAsync<string>("Wrong format");
            ...
            expect(newVal.error).toBe("Wrong format");
    ...
    describe("mapErr", () => {
        it("Maps an error using a synchronous function", async () => {
            ...
            expect(newVal.isErr()).toBe(true);
            expect(newVal.error).toBe("Error: Wrong format");
    ...
    describe("andThen", () => {
        ...
        it("Skips an Error", async () => {
            const asyncVal = errAsync<string>("Wrong format");

            const andThenResultFn = vitest.fn(() => ok<string>("good"));
    ...
    describe("inspect", () => {
        it("Calls the passed function but returns an original ok", async () => {
            ...
            const teed = await okVal.inspect(passedFn);
    ...
    describe("inspectErr", () => {
        it("Calls the passed function but returns an original err", async () => {
            ...
            const teed = await errVal.inspectErr(passedFn);
    ...
    describe("orElse", () => {
        it("Skips orElse on an Ok value", async () => {
            const okVal = okAsync(12);
            const errorCallback = vitest.fn(_errVal => errAsync<string>("It is now a string"));
    ...
    describe("unwrapOr", () => {
        ...
        it("returns a promise to the provided default value on an Error", async () => {
            const unwrapped = await errAsync<number>(12).unwrapOr(10);
    ...
    describe("fromPromise", () => {
        it("Accepts an error handler as a second argument", async () => {
            ...
            expect(val.error).toEqual(Error("Oops: No!"));
        });
    });
